...
for curItr = 1 : length(reducedDimArray)
    ...
    [fa_mappedX, fa_mapping] = compute_mapping(X, 'FA', reducedDim);
    recX_FA = reconstruct_data(fa_mappedX, fa_mapping);%recX = imN * imFeature
    %recX_PCA = reconstruct_data(recX_PPCA, pca_mapping);
    
    % change recX into imH * imW * imN
    recX = reshape(recX_FA', imH, imW, imN);
    
    %% plot reconstructed image
    titleStr = sprintf('FA: ReconstructedFrom%dDimensions',reducedDim);
    figure;
